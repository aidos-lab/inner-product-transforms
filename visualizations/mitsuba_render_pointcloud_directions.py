...
def create_state_dict(point_cloud, height, show_background=True):
    ...
    spheres = {}
    for i, (h, pt) in enumerate(zip(height, point_cloud)):
        # sphere_template = {
        #     "type": "sphere",
        #     "radius": 0.015,
        #     "bsdf": {
        ...
        template["center"] = [x, y, z]
        # print(h)
        if h < 0.02:
            template["bsdf"]["reflectance"]["value"] = GRAY
            # print("blud")
        else:
            # print("gray")
            template["bsdf"]["reflectance"]["value"] = COLOR

        spheres[f"sphere_{i}"] = template.copy()
...
v = generate_uniform_directions(num_thetas=32, d=3, seed=2025).numpy()
v[:, 16] = np.array([[-1, -2, 1]]) / np.sqrt(3)

heights = pts_normalized @ v
np.save("./results/heights.npy", heights[0])
...
for i in args.idx:
    ...
    state_dict = create_state_dict(
        point_cloud[i], show_background=show_bg, height=heights[i][:, 16]
    )
