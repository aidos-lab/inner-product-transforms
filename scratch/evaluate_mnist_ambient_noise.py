...
vis_x_original = {i: [] for i in range(10)}
...
for _ in range(10):
    ...
    batch.x += 0.05 * torch.randn_like(batch.x)

    noisy_ect = layer(batch, batch.batch)

    with torch.no_grad():
        recon_batch = ect_encoder_litmodel.model.forward(noisy_ect).cuda()

    for i in vis_x.keys():
        vis_x[i].append(recon_batch[idxs[i]])
        vis_ect[i].append(noisy_ect[idxs[i]])
        vis_x_original[i].append(batch[idxs[i]].x.clone())
...
torch.save(vis_x_original, f"./results/noise_ambient_vis_x_original.pt")
