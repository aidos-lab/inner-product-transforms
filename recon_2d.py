...
def fbp(ect):
    ...
    reconstruction_fbp = (
        iradon(sinogram, theta=theta, filter_name="ramp") * 100
    )
...
def get_edge_indices(reconstruction_fbp, x_hat, v):
...
if __name__ == "__main__":
    ...
    # x = data.x
    # ei = data.edge_index
    x = torch.tensor([[0.6, 0], [0.0, 0.5], [-0.4, 0.4], [0, 0]])
    ...
    data_true = Data(x=x, edge_index=ei)
    g = torch_geometric.utils.to_networkx(data_true, to_undirected=True)
    nx.draw(g, pos=data_true.x.numpy())
    plt.show()
