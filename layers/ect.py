...
def compute_ect_points(
    batch: Batch, index, v: torch.FloatTensor, lin: torch.FloatTensor
):
    ...
    """
    nh = batch.x @ v
    return compute_ecc(nh, index, lin)
...
class EctLayer(nn.Module):
    ...
    def forward(self, batch: Batch, index):
        ...
        ect = self.compute_ect(batch, index, self.v.movedim(-1, -2), self.lin)
