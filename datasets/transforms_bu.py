...
class EctTransform:
    def __init__(self):
        self.layer = EctLayer(config = EctConfig(num_thetas=32,bump_steps=32),fixed=True)
    ...
    def __call__(self,data):
        ...
        # given_min = n - n * (n - 1) / 2
        # given_max = n
        # scaled_ect = (ect - given_min) / (given_max - given_min) * (interval_max - interval_min) + interval_min
